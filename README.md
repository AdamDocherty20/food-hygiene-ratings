...
| `ADMIN_PASSWORD` | required for `/admin` | Gates the business-claim review area (see below). Without it, `/admin/*` is unreachable — visiting redirects to a login page that can never succeed. |
| `BLOB_READ_WRITE_TOKEN` | required for photo uploads in `/admin` | Auto-set once you connect a Vercel Blob store to the project (Storage tab in the Vercel dashboard) — you don't set this by hand. Without it, the admin editor's photo upload fails; everything else in `/admin` still works. |
...
**Business owner claims.** A business owner can click "Claim this business" on their establishment's page to submit a contact request (`/establishment/[id]/claim`). Nothing goes live automatically — you review and approve/reject each claim yourself at `/admin/claims` (password-gated by `ADMIN_PASSWORD`), then add whatever description/website/phone/photos the owner sends you via `/admin/establishment/[id]`. There's no self-serve owner login yet by design — see that page's own comments for why. Photos there upload directly to Vercel Blob from your browser (needs `BLOB_READ_WRITE_TOKEN`, see above); Blob is free on Vercel's Hobby plan within its usage limits — if you ever exceed them, Vercel pauses Blob access for 30 days rather than billing you, so there's no surprise-invoice risk the way there is with the Places API above.
